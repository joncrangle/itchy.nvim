...
describe('itchy.adapters.go', function()
	...
	it('maps log output to stderr events', function()
		local ctx = ctx_for(FULL_SRC)
		local prepared = go_adapter.prepare(ctx)
		with_cleanup(prepared, nil, function()
			local nonce = prepared.metadata.nonce
			local result = {
				code = 0,
				signal = 0,
				stdout = '\30ITCHY:' .. nonce .. ':{"kind":"stderr","line":3,"message":"log-hi"}\n',
				stderr = '',
			}
			local events = go_adapter.decode(ctx, prepared, result)
			eq(#events, 1)
	...
	it('falls back to legacy when Tree-sitter is unavailable', function()
		local orig = go_adapter.has_treesitter
		go_adapter.has_treesitter = function()
			return false
		end
		local notified = {}
		...
		go_adapter.has_treesitter = orig
		assert(ok)
