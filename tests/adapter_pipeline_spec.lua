  ...
  it('prepare -> executor result -> decode -> render without touching parser internals', function()
    ...
    local mock_adapter = {
      ...
      prepare = function(ctx)
        return {
          source = ctx.source,
          metadata = { filetype = ctx.filetype },
        }
      end,
      decode = function(_, _, result)
        local events = {}
        for _, line in ipairs(vim.split(result.stdout or '', '\n')) do
          local msg = line:match '^OUT:(.*)'
          if msg then
            table.insert(events, event.create('stdout', msg, 2))
          end
        end
        return events
      end,
    ...
    local result = { code = 0, signal = 0, stdout = 'OUT:hello\n', stderr = '' }
